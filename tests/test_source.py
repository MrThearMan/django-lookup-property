...
def test_lookup_property__f_rel_one_to_one__source():
    assert Example.rel_one_to_one.func_source == cleandoc(
        """
        def rel_one_to_one(self):
            return self.thing.pk
        """,
    )


def test_lookup_property__f_rel_many_to_one__source():
    assert Example.rel_many_to_one.func_source == cleandoc(
        """
        def rel_many_to_one(self):
            return self.other.pk
        """,
    )


def test_lookup_property__f_rel_one_to_many__source():
    assert Example.rel_one_to_many.func_source == cleandoc(
        """
        @rel_one_to_many.override
        def _(self):
...
def test_lookup_property__f_rel_many_to_many__source():
    assert Example.rel_many_to_many.func_source == cleandoc(
        """
        @rel_many_to_many.override
        def _(self):
...
def test_lookup_property__q_gte__source():
    assert Example.q_gte.func_source == cleandoc(
        ...
        def q_gte(self):
            import datetime
            return self.timestamp >= datetime.datetime.now(tz=datetime.UTC)
        """,
    )
